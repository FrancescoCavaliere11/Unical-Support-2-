...
public class EmailClassifierImpl implements EmailClassifier {
    ...
    @Override
    public ClassificationResult classifyEmail(EmailData email) {
        try {
            String system = promptService.buildSystemMessage();
            String user   = promptService.buildUserMessage(email);
    ...
    @Override
    public List<ClassificationResult> classifyEmailBatch(List<EmailData> emails) {
        try {
            String system = promptService.buildSystemMessageBatch(); // Sei un LLM fai questo
            String user   = promptService.buildUserMessageBatch(emails); // Stringa con Email0..EmailN
